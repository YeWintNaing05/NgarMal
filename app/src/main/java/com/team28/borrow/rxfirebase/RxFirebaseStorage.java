...
public class RxFirebaseStorage {
        ...
        return Single.create(emitter -> {
            final StorageTask<UploadTask.TaskSnapshot> taskSnapshotStorageTask =
                    storageRef.putBytes(bytes)
                            .addOnSuccessListener(taskSnapshot -> emitter.onSuccess(taskSnapshot)).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            if (!emitter.isDisposed())
                                emitter.onError(e);
    ...
    //YWN samsung tech institute
    //for in app use
    @NonNull
    public static Single<Task> putBytesWithOnComplete(@NonNull final StorageReference storageRef,
                                                      @NonNull final byte[] bytes) {
        return Single.create(emitter -> storageRef.putBytes(bytes)
                .continueWithTask(task -> storageRef.getDownloadUrl())
                .addOnCompleteListener(emitter::onSuccess)
    ...
    @NonNull
    public static Single<Task<Uri>> putFileWithTask(@NonNull final StorageReference storageRef,
                                                    @NonNull final Uri uri) {
        return Single.create(emitter -> storageRef.putFile(uri)
                .continueWithTask(task -> storageRef.getDownloadUrl())
                .addOnCompleteListener(emitter::onSuccess)
                .addOnFailureListener(e -> {
                    if (!emitter.isDisposed())
