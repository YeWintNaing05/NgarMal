...
import static com.team28.borrow.rxfirebase.DocumentSnapshotMapper.DOCUMENT_EXISTENCE_PREDICATE;
import static com.team28.borrow.rxfirebase.DocumentSnapshotMapper.QUERY_EXISTENCE_PREDICATE;
...
public class RxFirestore {
    ...
    public static Single<DocumentReference> addDocument(@NonNull final CollectionReference ref,
        ...
        return Single.create(emitter -> ref.add(data).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
            @Override
    ...
    public static Single<DocumentReference> addDocument(@NonNull final CollectionReference ref,
        ...
        return Single.create(emitter -> ref.add(pojo).addOnCompleteListener(task -> {
            emitter.onSuccess(task.getResult());
            Log.e("data success", "data");
    ...
    public static Maybe<QuerySnapshot> getCollection(@NonNull final CollectionReference ref) {
        return Maybe.create(emitter -> ref.get().addOnSuccessListener(documentSnapshots -> {
    ...
    public static Maybe<QuerySnapshot> getCollection(@NonNull final Query query) {
        return Maybe.create(emitter -> query.get().addOnSuccessListener(documentSnapshots -> {
            if (documentSnapshots.isEmpty()) {
                emitter.onComplete();
            } else {
                emitter.onSuccess(documentSnapshots);
    ...
    public static Flowable<DocumentSnapshot> observeDocumentRef(@NonNull final DocumentReference ref,
                                                                ...
                                                                @NonNull BackpressureStrategy strategy) {
        return Flowable.create(emitter -> {
            final ListenerRegistration registration = ref.addSnapshotListener(executor, metadataChanges, (documentSnapshot, e) -> {
                if (e != null && !emitter.isCancelled()) {
                    emitter.onError(e);
                    return;
                }
    ...
    public static Flowable<DocumentSnapshot> observeDocumentRef(@NonNull final DocumentReference ref,
                                                                ...
                                                                @NonNull BackpressureStrategy strategy) {
        return Flowable.create(emitter -> {
            final ListenerRegistration registration = ref.addSnapshotListener(activity, metadataChanges, (documentSnapshot, e) -> {
                if (e != null && !emitter.isCancelled()) {
                    emitter.onError(e);
                    return;
                }
    ...
    public static Flowable<QuerySnapshot> observeQueryRef(@NonNull final Query ref,
                                                          ...
                                                          @NonNull BackpressureStrategy strategy) {
        return Flowable.create(emitter -> {
            final ListenerRegistration registration = ref.addSnapshotListener(metadataChanges, (querySnapshot, e) -> {
                if (e != null && !emitter.isCancelled()) {
                    emitter.onError(e);
                    return;
                }
    ...
    //YWN samsung tech  use for this project
    @NonNull
    public static Flowable<QueryDocumentSnapshot> observeQueryRefByRow(@NonNull final Query ref,
                                                                       @NonNull final MetadataChanges metadataChanges,
                                                                       @NonNull BackpressureStrategy strategy) {
        return Flowable.create(emitter -> {
            final ListenerRegistration registration = ref.addSnapshotListener(metadataChanges, (querySnapshot, e) -> {
    ...
    //add document by specified document reference
    @NonNull
    public static Completable addDocument(@NonNull final DocumentReference ref,
